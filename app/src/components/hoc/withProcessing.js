        ...
        if (data.srapi.hasOwnProperty('findManyCourse')) {
            ...
            if (processed.courses.length > 0) {
                if (processed.courses[0].hasOwnProperty('block')) {
                    const blocks = removeDupes(processed.courses.map(node => node.block));
                ...
                if (processed.courses[0].hasOwnProperty('code')) {
                    processed.codes = removeDupes(processed.courses.map(node => node.code));
                }

                if (processed.courses[0].hasOwnProperty('name')) {
                    processed.courseNames = removeDupes(new Set(processed.courses.map(node => node.name));
                }

                if (processed.courses[0].hasOwnProperty('key')) {
                    processed.courseKeys = removeDupes(new Set(processed.courses.map(node => node.key));
                }

                if (processed.courses[0].hasOwnProperty('department')) {
                    const departments = removeDupes(processed.courses.map(node => node.department));
                ...
                if (processed.courses[0].hasOwnProperty('teacher')) {
                    processed.teacherNames = removeDupes(processed.courses.map(node => node.teacher));
                }

                if (processed.courses[0].hasOwnProperty('semester')) {
                    processed.semesters = removeDupes(processed.courses.map(node => node.semester)).sort((a, b) => {
